...
public class DietPage {
    ...
    private ComboBox<String> redMeatFrequencyComboBox; private ComboBox<String> poultryFrequencyComboBox; private ComboBox<String> fishSeafoodFrequencyComboBox;
    private TextField dailyRiceServingsField; private ComboBox<String> riceTypeComboBox;
    private ComboBox<String> dietaryPatternComboBox; private TextField weeklyFoodWasteField; private ComboBox<String> localFoodSourcingComboBox;
    private Label errorMessageLabel;
    public DietPage(MainController controller, UserData userData) { this.controller = controller; this.userData = userData; }
    public Node getView() {
        ...
        Label title = new Label("Dietary Habits"); title.setFont(Font.font("Arial", FontWeight.BOLD, 18));
        GridPane dietGrid = createStyledGridPane();
        String[] frequencyOptions = {"Daily", "3-5 times/week", "1-2 times/week", "Rarely/Never"};
        redMeatFrequencyComboBox = new ComboBox<>(); redMeatFrequencyComboBox.getItems().addAll(frequencyOptions); redMeatFrequencyComboBox.setPromptText("Select frequency");
        addLabeledControl(dietGrid, "Red Meat Consumption:", redMeatFrequencyComboBox, 0, "Frequency of consuming red meat (beef, pork, etc.).");
        poultryFrequencyComboBox = new ComboBox<>(); poultryFrequencyComboBox.getItems().addAll(frequencyOptions); poultryFrequencyComboBox.setPromptText("Select frequency");
        ...
        fishSeafoodFrequencyComboBox = new ComboBox<>(); fishSeafoodFrequencyComboBox.getItems().addAll(frequencyOptions); fishSeafoodFrequencyComboBox.setPromptText("Select frequency");
        ...
        dailyRiceServingsField = new TextField(); dailyRiceServingsField.setPromptText("e.g., 2");
        ...
        riceTypeComboBox = new ComboBox<>(); riceTypeComboBox.getItems().addAll("Irrigated", "Rainfed", "Unknown/Average"); riceTypeComboBox.setPromptText("Select rice type");
        addLabeledControl(dietGrid, "Rice Type (if known):", riceTypeComboBox, 4, "Type of rice predominantly consumed, if known.");
        dietaryPatternComboBox = new ComboBox<>(); dietaryPatternComboBox.getItems().addAll("Meat-eater (Typical)", "Reduced Meat / Flexitarian", "Vegetarian (No meat/fish)", "Vegan (No animal products)");
        ...
        weeklyFoodWasteField = new TextField(); weeklyFoodWasteField.setPromptText("e.g., 0.5");
        addLabeledControl(dietGrid, "Weekly Food Waste (kg):", weeklyFoodWasteField, 6, "Estimated kilograms of food wasted per week.");
        localFoodSourcingComboBox = new ComboBox<>(); localFoodSourcingComboBox.getItems().addAll("Often", "Sometimes", "Rarely", "Not Sure/Don't Track");
        ...
        Button saveButton = new Button("Save & Next (Transport)"); saveButton.setStyle("-fx-background-color: #66BB6A; -fx-text-fill: white; -fx-padding: 8 15; -fx-background-radius: 5;");
        saveButton.setOnAction(_ -> saveDataAndProcess());
        VBox buttonAndErrorBox = new VBox(10, errorMessageLabel, saveButton); buttonAndErrorBox.setAlignment(Pos.CENTER_LEFT); buttonAndErrorBox.setPadding(new Insets(10,0,0,0));
        mainLayout.getChildren().addAll(title, dietGrid, buttonAndErrorBox);
        ScrollPane scrollPane = new ScrollPane(mainLayout); scrollPane.setFitToWidth(true); scrollPane.setStyle("-fx-background-color: transparent; -fx-background: #F1F8E9;");
        return scrollPane;
    }
    ...
    private void saveDataAndProcess() {
        ...
        userData.setRedMeatFrequency(redMeatFrequencyComboBox.getValue()); userData.setPoultryFrequency(poultryFrequencyComboBox.getValue());
        userData.setFishSeafoodFrequency(fishSeafoodFrequencyComboBox.getValue()); userData.setRiceType(riceTypeComboBox.getValue());
        userData.setDietaryPattern(dietaryPatternComboBox.getValue()); userData.setLocalFoodSourcingFrequency(localFoodSourcingComboBox.getValue());
        try {
        ...
        if (dataIsValid) { System.out.println("Diet Page: Data saved to UserData model."); controller.processDietData(); }
    }
    private double parseDoubleField(TextField field, String fieldName, boolean isRequired) throws IllegalArgumentException {
        String text = field.getText(); if (text == null || text.trim().isEmpty()) { if (isRequired) throw new IllegalArgumentException(fieldName + " cannot be empty."); return 0.0; }
    ...
    private void appendErrorMessage(String message) { if (errorMessageLabel.getText().isEmpty()) errorMessageLabel.setText(message); else errorMessageLabel.setText(errorMessageLabel.getText() + "\n" + message); }
}
