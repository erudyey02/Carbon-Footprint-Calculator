...
public class CalculationServiceImpl implements CalculationService {
    ...
    public CalculatedFootprintData calculateFootprint(UserData userData) {
        ...
        double wasteFootprint = calculateWasteFootprint(userData); // Call new waste calculation
    ...
    private double calculateTransportationFootprint(UserData userData) {
        ...
        if (userData.getRegularTransportEntries() != null) {
            for (TransportEntry entry : userData.getRegularTransportEntries()) {
                double annualDistanceKm = entry.getDistanceKm();
        ...
        if (userData.getFlightEntries() != null) {
            for (FlightEntry flight : userData.getFlightEntries()) {
                ...
                double flightFactor = 0; String haulTypeSimple = flight.getHaulType();
                if ("Short".equalsIgnoreCase(haulTypeSimple)) flightFactor = AppConstants.EF_TRANSPORT_FLIGHT_SHORT_HAUL_ECONOMY_RFI3_KGCO2E_PER_PKM;
                else if ("Medium".equalsIgnoreCase(haulTypeSimple)) flightFactor = AppConstants.EF_TRANSPORT_FLIGHT_MEDIUM_HAUL_ECONOMY_RFI3_KGCO2E_PER_PKM;
                else if ("Long".equalsIgnoreCase(haulTypeSimple)) flightFactor = AppConstants.EF_TRANSPORT_FLIGHT_LONG_HAUL_ECONOMY_RFI3_KGCO2E_PER_PKM;
                if ("Business".equalsIgnoreCase(flight.getCabinClass())) flightFactor *= AppConstants.FLIGHT_BUSINESS_CLASS_MULTIPLIER;
